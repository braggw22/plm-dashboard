...
const DateService = (() => {
  ...
  const now = () => new Date(); // system clock; all formatting uses TZ
...
const DefaultData = (() => {
  ...
  return {
    ...
    tasks: [
      { id: "t1", title: "Finalize BLDG 1289 drawing corrections", owner: "Weston", dueISO: DateService.iso(today), priority: "High", status: "In Progress" },
      { id: "t2", title: "Review PRTH TFR submittal", owner: "Team", dueISO: DateService.iso(today), priority: "Medium", status: "Not Started" },
      { id: "t3", title: "Draft RFI list for PRTH", owner: "Weston", dueISO: DateService.iso(DateService.yesterday(today)), priority: "Low", status: "Not Started" }
    ],
    tfr: [],
    rfis: [],
    risks: []
...
function renderDashboard() {
  ...
  const myTasksDueToday = DB.tasks.filter(t =>
    t.dueISO === DateService.iso(today) && /weston/i.test(t.owner || "")
  ).length;
  ...
  const html = `
    <section class="card">
      ...
      <div class="kpi-row" role="group" aria-label="Dashboard KPIs">
        <div class="kpi" tabindex="0" aria-label="My tasks due today">
          <div class="label">My Tasks Due Today</div>
          <div class="num">${myTasksDueToday}</div>
        </div>
        <div class="kpi" tabindex="0" aria-label="Team tasks next 7 days">
          <div class="label">Team Tasks (7d)</div>
          <div class="num">${teamNext7}</div>
        </div>
        <div class="kpi" tabindex="0" aria-label="RFIs">
          <div class="label">RFIs</div>
          <div class="num">${DB.rfis.length}</div>
        </div>
        <div class="kpi" tabindex="0" aria-label="Risks">
    ...
    <section class="card">
      ...
      <p class="label">Phase 1 placeholder. Hooked up in Phase 2.</p>
...
function renderTagUp() {
  ...
  const yestISO = DateService.iso(yest);
  ...
  const suggestions = DB.tasks
    .filter(t => t.dueISO === todayISO)
    .map(t => `• ${t.title} (${t.owner || "Unassigned"})`);
  ...
  const html = `
    <section class="card">
      ...
      <p class="label" style="margin-top:0.5rem;">Phase 1 saves in memory. File write to JSON will be handled in Phase 2 with a backend or SharePoint/Access bridge.</p>
    </section>
  `;
    ...
    return `
      <li class="card" tabindex="0">
        <div class="inline">
          ...
          <span class="label">Owner: ${t.owner || "—"}</span>
